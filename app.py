...
def get_registry(league_id: int, season: int):
...
season = st.sidebar.text_input("Temporada", value="2025", help="Ex.: 2025 ou 2023-2024")
...
st.markdown('<div class="hero"><div class="eyebrow">Sports intelligence / 03</div><h1>O jogo,<br>bem lido.</h1><p>Uma inteligência de análise que percorre a temporada rodada a rodada, consolida os jogos e executa as hipóteses sobre o conjunto completo — sem banco de dados.</p></div>', unsafe_allow_html=True)
...
if st.sidebar.button("Buscar temporada e rodar hipóteses", type="primary", use_container_width=True):
    if end_round < start_round:
        ...
        try:
            registry = get_registry(int(league_id), int(season.split('-')[0]))
            ...
            for index, round_number in enumerate(rounds, start=1):
                ...
                if round_matches:
                    empty_streak = 0
                    unique_matches.update({match.id: match for match in round_matches})
                else:
                    empty_streak += 1
                progress.progress(index / len(rounds), text=f"Rodada {round_number}/{rounds[-1]} · {len(unique_matches)} partidas encontradas")
            ...
            st.session_state["rounds_collected"] = index
            st.sidebar.success(f"{len(matches)} partidas; {len(evidence)} evidências avaliadas.")
...
if not matches:
    ...
    with right:
        ...
        st.caption(f"{len(upcoming)} partidas sem placar e {st.session_state.get('rounds_collected', 0)} rodadas percorridas.")
