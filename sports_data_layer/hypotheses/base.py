...
class Discovery:
    ...
    q_value: float | None = None       # p-value ajustado aproximado pelo lote
    effect_size: float | None = None   # magnitude do efeito, quando a hipótese fornece
    evidence_status: str = "candidate" # candidate, validated, replicated, rejected
...
def apply_multiple_comparisons_correction(discoveries: list[Discovery], alpha: float = 0.05) -> list[Discovery]:
    ...
    ordered = sorted(range(len(discoveries)), key=lambda index: discoveries[index].p_value)
    q_values = [None] * len(discoveries)
    minimum = 1.0
    for rank in range(len(ordered) - 1, -1, -1):
        index = ordered[rank]
        value = min(minimum, discoveries[index].p_value * len(discoveries) / (rank + 1))
        minimum = value
        q_values[index] = value

    for discovery, is_significant, q_value in zip(discoveries, flags, q_values):
        discovery.adjusted_significant = is_significant
        discovery.q_value = q_value
        discovery.evidence_status = "candidate" if is_significant else "exploratory"
